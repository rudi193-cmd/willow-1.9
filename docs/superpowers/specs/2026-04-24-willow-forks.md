...
## Open Questions (for Sean to decide)

1. **Auto-fork on session start?** Should every Claude Code session automatically
   create a fork, or is fork creation explicit (opt-in)?

2. **Fork expiry?** Should open forks expire after N days with no activity?
   Or does everything stay open indefinitely until explicitly resolved?

3. **Trust level for merge/delete?** Can Hanuman merge its own forks, or does
   every fork require Sean's explicit approval to merge?

4. **Grove channel vs fork?** Is a Grove channel a long-lived fork, or are they
   different primitives? (Channels = permanent; fork threads = ephemeral?)

5. **Migration of existing work?** The 33 atoms we ingested today — retroactively
   assign them to a fork? Or start clean from this spec forward?
